...
from timm.models.layers import trunc_normal_ # trunc_normal_截断正态分布
...
if __name__ == '__main__':
    ...
    batch_size = 16
    seq_length = 15
    H, W, channel = 48, 48, 32
    rpe = True

    # 创建 MultiheadAttentionRPE 实例
    # 假设 embed_dim 和 num_heads 分别为定义好的值
    embed_dim = channel
    num_heads = 4
    model = MultiheadAttentionRPE(embed_dim, num_heads, rpe=rpe, window_size=48)

    # 生成模拟输入数据
    # 假设 input_data 是一个随机生成的 4D 张量，形状为 [batch_size, seq_length, H, W, channel]
    input_data = torch.randn(batch_size, seq_length, H, W, channel)

    # 转换输入数据形状
    input_data = input_data.view(batch_size * seq_length, H * W, channel).transpose(0, 1)

    # 执行 MultiheadAttentionRPE 的 forward 运算
    # 注意：这里为了简化示例，假设 key 和 value 与 query 相同，实际应用中 key 和 value 可以是不同的输入
    # 使用 padding mask 和 attention mask 来限制注意力的范围和遮蔽无效区域
    # 也可以获取 attention 输出权重和 query, key 以供需要的用途
    attn_output, _, _, attn_output_weights = model(
        query=input_data,
        key=input_data,
        value=input_data,
        key_padding_mask=None,
    ...
    # 打印注意力输出张量形状和注意力权重张量形状
    print("Attention Output Shape:", attn_output.shape)
    print("Attention Output Weights Shape:", attn_output_weights.shape)
